"""LLM dispatch layer: Claude CLI, Ollama, and prompt utilities."""
...
def _run_llm(
    ...
    """Dispatch to Claude CLI or Ollama and return generated text.
...
def _run_claude_cli(
    ...
    """Call the Claude CLI in ``--print`` mode and return the response.

    The prompt is piped via stdin so the process never blocks waiting for
    interactive input.
